<form action="/productsActions/createProduct.php" method="POST">
    <div>
        ...
        <label class="product_label">product_name:
            <input class="product_name" type="text" name="name" value="{{NAME}}">
        </label>
    </div>
    <div>
        <label class="product_label">product_price:
            <input class="product_price" type="text" name="price" value="{{PRICE}}">
        </label>
    </div>
    <div>
        <label class="product_label">product_image:
            <input class="product_img" type="text" name="image" value="{{IMAGE}}">
        </label>
    </div>
    <div>
        <!-- для textarea значение по умолчанию выглядит так -->
        <label class="product_label">product_description:
            <textarea class="product_description" name="description" cols="100" rows="30">{{DESCRIPTION}}</textarea>
        </label>
    </div>
    <div>
